...
class server(models.Model):
    ...
    def hourly_sessions(self):
        ...
        cursor.execute("SELECT `hour`, FLOOR(max(`cnt`)) AS `max`, FLOOR(AVG(`cnt`)) AS `users` FROM `server_hourlystats` where `server_id` = %s and DATE(`day`) >= (now() - INTERVAL 2 DAY) GROUP BY `hour` ORDER BY `hour`", [self.pk])
